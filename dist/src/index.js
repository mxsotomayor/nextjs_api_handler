import { cache } from "./cache/lmdb/index.js";
import { ErrorResponse, SuccessResponse } from "./http/responses.js";
import { cookies, headers } from "next/headers";
import { buildJSONBody } from "./helpers.js";
import settings from "./settings.json";
import { ErrorMetaCodes, RouteFunctionError, } from "./types.js";
import { FORM_ENC_TYPES } from "./constants.js";
import { HttpHeaders } from "./http/headers.js";
export const __VERSION = settings.version;
export const __ALLOW_CORS = settings.cors;
const routeIsRegistered = (_routeKey) => {
    if (!_routeKey || !globalRoutes[_routeKey]) {
        throw new RouteFunctionError(`Invalid request. route_key invalid or missing, given value '${_routeKey}'.`, { code: ErrorMetaCodes.ROUTE_KEY_NOT_REGISTERED });
    }
    return true;
};
const unpackRequest = async (request) => {
    ...
    let routeKeyname = "", requestData = null;
    ...
    if (formContentType === FORM_ENC_TYPES["form-data"]) {
        ...
        routeKeyname = formData.get("route_key") ?? "";
    ...
    const builtRoute = await Promise.resolve(globalRoutes[routeKeyname]?.(requestData, request));
    if (builtRoute == null)
        return null;
    return {
        ...builtRoute,
...
const executeRequest = async (unpackedRequest) => {
    ...
    let responseData;
    const isSuccess = response.ok, statusCode = response.status;
    try {
...
//
const printErrorRequest = (unpackedRequest) => {
 ...
 */
export const init = (NextjsAPIRoutesProps) => {
    globalRoutes = NextjsAPIRoutesProps;
    console.log(`🚀 - Routes initialized (${Object.keys(globalRoutes).length}) found`);
};
export async function dispatchHandler(request) {
    try {
        ...
        let response = {};
        console.log(`⚡ QUERYING URL [${unpackedRequest.method ?? "GET"}] - ${unpackedRequest.url}`);
        // processing cache
        if (unpackedRequest.cacheOptions) {
            response = await cache(unpackedRequest.cacheOptions.cacheKey, () => executeRequest(unpackedRequest), unpackedRequest.cacheOptions.maxAge);
        }
        else {
        ...
        if (unpackedRequest.onSuccess) {
            return SuccessResponse(await unpackedRequest.onSuccess(response.success, request));
        }
        return SuccessResponse(response.success);
    }
    catch (e) {
        if (e instanceof RouteFunctionError) {
            return ErrorResponse({ error: e.message + " Error Code: " + e.meta.code }, 400);
        }
        const _error = e;
